...
def make_steps(n_trials: int, smoke: bool, gpu_concurrency: int) -> list[dict]:
    """Return ordered list of step dicts. Order = priority for scheduling."""
    nt = 1 if smoke else n_trials
    p1 = 5 if smoke else 30  # phase1 epochs
    p2 = 5 if smoke else 50  # phase2 epochs
    seeds = [42] if smoke else [42, 1337, 7]
    nw_feat = 2
    nw_raw = 4 if smoke else 8
    ...
    def opt_step(slug, arch, variant, num_workers):
        run_dir = RUN_DIR_BASE / f"optuna_clean-{variant}-{arch}"
    ...
    steps = [
        # Feature NN — light, run first to fail-fast
    ...
    # RF on CPU — independent
    rf_run_dir = RUN_DIR_BASE / "optuna_clean-rf"
...
def main():
    ...
    ap.add_argument("--smoke-test", action="store_true",
                    help="1 trial, 5 epochs each — verifies all 7 launch")
    ap.add_argument("--max-gpu-jobs", type=int, default=2,
                    help="Concurrent NN training jobs on GPU (default 2)")
    args = ap.parse_args()
