"""V12 per-session RPE plots — actual vs predicted per set, validation only.
...
Models (v12 winners; equal-weight Optuna score):
  feat-MLP @ 2s             (best phase F1 = 0.501)
  fatigue-raw-tcn @ 2s      (best fatigue r = +0.429)
  feat-MLP @ 1s             (lowest composite multi-task score 0.4004)

Predictions come from LOSO 7-fold validation (test_preds.pt × 3 seeds × 7 folds).
Output: runs/comparison_v12/per_session_rpe/
"""
...
OUT = ROOT / "runs" / "comparison_v12" / "per_session_rpe"
...
MODELS = [
    ('feat-MLP @ 2s (best phase)',     'optuna_clean_v12eqw-w2s-multi-feat-mlp',  'features', 2.0),
    ('fatigue-raw-tcn @ 2s (best r)',  'optuna_clean_v12eqw-w2s-fatigue-raw-tcn', 'raw',      2.0),
    ('feat-MLP @ 1s (best composite)', 'optuna_clean_v12eqw-w1s-multi-feat-mlp',  'features', 1.0),
]
...
def main():
    ...
    for label, d in model_data.items():
        ...
        _plot(axes[0], best_sets,  f'Best: {SUBJECT_TO_REC.get(best_subj, best_subj)}')
        _plot(axes[1], worst_sets, f'Worst: {SUBJECT_TO_REC.get(worst_subj, worst_subj)}')
        _plot(axes[2],
