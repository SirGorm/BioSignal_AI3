"""Inspect recording 014 in its clock-aligned form.

Source: dataset_aligned/recording_014/
  - biosignals (ecg, emg, eda, ax/ay/az, ppg_*) copied verbatim from dataset/
    — already on dataset (PC) clock, no shift.
  - temperature.csv shifted from dataset_memory/ by per-recording offset
    (memory_clock - 2598.984 s = dataset_clock) and trimmed to dataset range.
  - markers.json + metadata.json + recording_NN_joints.json copied verbatim
    from dataset/ (already on dataset clock).
  - clock_alignment.json (per-recording offset metadata)

Output: inspections/recording_014_aligned/{report.md, findings.md, stats.json, *.png}

Read-only. No filtering, resampling, or feature extraction.
"""
...
REC_ID = "014"
REC_DIR = REPO / "dataset_aligned" / f"recording_{REC_ID}"
OUT_DIR = REPO / "inspections" / f"recording_{REC_ID}_aligned"
...
def participants_lookup(rec_num: int) -> dict:
    ...
    i = idx[0]
    exercises = [
        str(v) for v in df.iloc[i, 2:].tolist() if pd.notna(v) and str(v) != "nan"
    ]
    rpe_row_vals = df.iloc[i + 1, 2:].tolist()
    rpe = [int(v) if pd.notna(v) else None for v in rpe_row_vals]
    rpe = [r for r in rpe if r is not None][: len(exercises)]
    return {"name": str(df.iloc[i]["Name:"]), "exercises": exercises, "rpe": rpe}


def parse_markers(markers_path: Path):
    ...
    for m in data["markers"]:
        ...
        if label.endswith("_Start") and "Set:" in label:
            ...
            cur = {"set_number": n, "start_unix": m["unix_time"], "end_unix": None, "reps": []}
        elif label.startswith("Set_") and label.endswith("_End"):
            n = int(label.split("_")[1])
            if cur and cur["set_number"] == n:
                cur["end_unix"] = m["unix_time"]
                sets.append(cur)
                cur = None
        elif "_Rep:" in label:
            ...
            if cur and cur["set_number"] == set_n:
                cur["reps"].append(m["unix_time"])
    return sets, reps


def detect_active_segments_from_acc(t, mag, fs, threshold=0.3):
    if len(t) < int(60 * fs):
        return []
    ...
    for i in range(n):
        ...
        if e_t - s_t > 10:
            segs.append((float(s_t), float(e_t)))
    return segs
...
def joint_coverage(rec_dir: Path, sets: list[dict]) -> list[dict]:
    coverage = []
    for s in sets:
        candidates = [rec_dir / f"recording_{s['set_number']:02d}_joints.json"]
...
def main() -> None:
    ...
    print(f"=== Inspecting {REC_DIR} ===")
    print(f"Output: {OUT_DIR}")

    with open(REC_DIR / "metadata.json") as f:
        meta = json.load(f)
    declared_fs = meta["sampling_rates"]
    print(f"Declared fs: {declared_fs}")

    with open(REC_DIR / "clock_alignment.json") as f:
        clock = json.load(f)
    print(
        f"Clock alignment: offset={clock['offset_s_memory_minus_dataset']} s, "
        f"pearson={clock['verification_pearson']}, probes={clock['verification_probes_agree']}"
    )

    rec_num = int(REC_ID)
    participants = participants_lookup(rec_num)
    print(f"Participant: {participants['name']}  exercises={participants['exercises']}  rpe={participants['rpe']}")

    sets, reps = parse_markers(REC_DIR / "markers.json")
    print(f"Markers: {len(sets)} sets, {len(reps)} reps")
    ...
    print(f"Acc-mag detected segments: {len(acc_segments)} (markers={len(sets)})")

    j_cov = joint_coverage(REC_DIR, sets)

    bio_t0 = channels["ecg"]["t"][0] if channels["ecg"].get("present") else 0.0
    ...
    plt.suptitle(f"recording_014 (aligned) — full session ({participants['name']})")
    plt.tight_layout()
    despine()
    ...
    if sets:
        ...
        for ax, mod in zip(axes, plot_channels):
            if mod == "acc_mag":
                ...
                t = channels[mod]["t"]; v = channels[mod]["x"]
        ...
        plt.suptitle("recording_014 — set 1 zoom (red lines = rep markers)")
        plt.tight_layout()
        despine()
        plt.savefig(OUT_DIR / "signal_zoomed_set1.png", dpi=110)
        plt.close()

    # 3) ppg_channel_check.png — 30 s window from minute 1
    ...
    if len(ppg_mods) >= 2:
        ...
        ax.set_title("PPG wavelengths (30 s rest @ ~min 1) — recording_014")
        ax.legend()
        plt.tight_layout()
        despine()
    ...
    for mod in plot_channels:
        if mod == "acc_mag":
            x = acc_mag; fs_c = fs_acc
        else:
            x = channels[mod]["x"]; fs_c = stats["channels"][mod]["fs_hz"]
        x = x[~np.isnan(x)]
    ...
    # 5) timestamp_alignment.png — bio coverage vs marker set windows
    fig, ax = plt.subplots(figsize=(14, 3))
    if channels["ecg"].get("present"):
        t = channels["ecg"]["t"]
        ax.scatter((t[::5000] - bio_t0) / 60, np.ones(len(t[::5000])), s=2, alpha=0.5, label="ecg samples")
    for s in sets:
        ax.axvspan((s["start_unix"] - bio_t0) / 60, (s["end_unix"] - bio_t0) / 60, color="C2", alpha=0.2)
    ax.set_yticks([1])
    ax.set_yticklabels(["bio"])
    ax.set_xlabel("Session time [min]")
    ax.set_title(
        f"Timestamp coverage (Unix anchored) — bio start "
        f"{datetime.fromtimestamp(bio_t0, tz=timezone.utc):%Y-%m-%d %H:%M:%S} UTC"
    )
    plt.tight_layout()
    despine()
    ...
    for cov, s in zip(j_cov, sets):
        ...
        ax.text(cx, 0.5, f"{cov['n_frames']} frames\n{cov['joint_file'] or 'MISSING'}",
                ha="center", va="center", fontsize=7,
                color="black" if cov["joint_file"] else "red")
    ax.set_xlabel("Session time [min]")
    ax.set_ylim(0, 1)
    ax.set_yticks([])
    ax.set_title("Joint-skeleton coverage per set — recording_014 (aligned)")
    plt.tight_layout()
    despine()
    ...
    stats["sets_from_participants"] = len(participants["exercises"])
    stats["bio_t0_unix"] = float(bio_t0)
    stats["bio_duration_s"] = float(channels["ecg"]["t"][-1] - bio_t0) if channels["ecg"].get("present") else None
    stats["joint_coverage"] = j_cov
    stats["sets"] = [
        {"set_number": s["set_number"], "start_unix": s["start_unix"], "end_unix": s["end_unix"],
         "duration_s": s["end_unix"] - s["start_unix"], "n_reps": len(s["reps"])}
        for s in sets
    ]
    stats["participants"] = participants
    stats["acc_segments"] = acc_segments
    ...
    for mod in ["ecg", "emg", "eda", "temperature", "ppg_green", "ax", "ay", "az"]:
        ...
        if cs.get("status") == "empty_or_missing":
            ...
            rows_hw.append(
                f"| {mod} | {cs.get('fs_hz', float('nan')):.2f} Hz | {cs.get('mean', 0):.3g} | "
                f"{cs.get('p1', 0):.3g} .. {cs.get('p99', 0):.3g} | {cs.get('pct_nan', 0):.2f}% |"
            )
    if "acc_mag" in stats["channels"]:
        cs = stats["channels"]["acc_mag"]
        rows_hw.append(
            f"| acc_mag | {cs.get('fs_hz', float('nan')):.2f} Hz | {cs.get('mean', 0):.3g} | "
            f"{cs.get('p1', 0):.3g} .. {cs.get('p99', 0):.3g} | {cs.get('pct_nan', 0):.2f}% |"
        )

    pad_ex = list(participants["exercises"]) + [None] * (len(stats["sets"]) - len(participants["exercises"]))
    pad_rpe = list(participants["rpe"]) + [None] * (len(stats["sets"]) - len(participants["rpe"]))
    set_rows = []
    for s, ex, rpe in zip(stats["sets"], pad_ex, pad_rpe):
        set_rows.append(f"| {s['set_number']} | {ex} | {rpe} | {s['duration_s']:.1f} s | {s['n_reps']} |")

    j_rows = [f"| {cov['set_number']} | {cov['joint_file'] or 'MISSING'} | {cov['n_frames']} |" for cov in j_cov]

    report = f"""# Inspection report — recording_014 (clock-aligned)

Source: `dataset_aligned/recording_014/`
- Biosignals (ecg/emg/eda/ax/ay/az/ppg_*) copied verbatim from `dataset/recording_014/` (already on PC clock).
- `temperature.csv` shifted from `dataset_memory/recording_014_memory/` by
  {clock['offset_s_memory_minus_dataset']:.3f} s (memory_clock - offset = dataset_clock); pearson
  {clock['verification_pearson']:.3f}, probes {clock['verification_probes_agree']},
  confidence {clock['verification_confidence']}.
- `markers.json` / `metadata.json` / `recording_NN_joints.json` passthrough from `dataset/`.
...
Total reps (markers.json): {len(reps)}
...
- `signal_overview.png` — all channels with marker-set windows shaded
